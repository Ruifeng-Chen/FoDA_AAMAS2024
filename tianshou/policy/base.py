...
class BasePolicy(ABC, nn.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def update(
        ...
        fast_buffer: Optional[ReplayBuffer],
        **kwargs: Any,
    ) -> dict[str, Any]:
        ...
        self.updating = True
        fast_batch, fast_indices = fast_buffer.sample(sample_size)
        lfiw_result = self.lfiw_learn(batch, fast_batch)
        batch = self.process_fn(batch, buffer, indices)
        result = self.learn(batch, **kwargs)
        
        result.update(lfiw_result)

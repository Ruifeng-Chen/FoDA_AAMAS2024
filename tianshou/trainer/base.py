...
class BaseTrainer(ABC):
    ...
    def run(self) -> dict[str, Union[float, str]]:
        ...
        try:
            self.is_run = True
            deque(self, maxlen=0)  # feed the entire iterator into a zero-length deque create an instance for this class to initiate the iteration
            info = gather_info(
                self.start_time,
                self.train_collector,
                self.test_collector,
    ...
    def _sample_and_update(self, buffer: ReplayBuffer,fast_buffer: ReplayBuffer, data: dict[str, Any]) -> None:
        ...
        losses = self.policy.update(sample_size=self.batch_size, buffer=buffer, fast_buffer = fast_buffer)
...
class OffpolicyTrainer(BaseTrainer):
    ...
    def policy_update_fn(self, data: dict[str, Any], result: dict[str, Any]) -> None:
        ...
        for _ in range(num_updates):
            self._sample_and_update(self.train_collector.buffer, self.train_collector.fast_buffer, data)
